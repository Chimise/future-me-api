...
export class AuthService {
    ...
    public async signup(email: string, password: string, role?: Role, id?: string) {
        let user = await this.userService.findByEmail(email);
        if (user) {
            throw new BadRequestException('An account with this email already exist');
        }
        const hashedPassword = await this.hashPassword(password)
        user = await this.userService.create({ email, password: hashedPassword, role, id });
        return user;
    }
    ...
    public getRoleFromToken(token: string): Role {
